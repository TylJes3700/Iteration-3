      ...
      return 
         new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
               ...
               // Create order
               Order order = new Order(id, Store.getCheckoutController().getTotal(), Store.getCheckoutController().getTax(), dateFormat.format(new Date()).toString().substring(0,10), dateFormat.format(new Date()).toString().substring(11,19), "cash", Store.dataAdapter.getName(), Store.getCheckoutController().getProducts());
               if (Store.dataAdapter.saveOrder(order)) {
                  Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "Print Receipt?", new ButtonType("YES"), new ButtonType("NO"));
                  Optional<ButtonType> result = alert.showAndWait();
                  if (result.isPresent() && result.get().getText() == "YES") {
                     alert = new Alert(Alert.AlertType.NONE, "Choose file type", new ButtonType("txt"), new ButtonType("html"));
                     result = alert.showAndWait();
                     if (result.isPresent()) {
                        Store.dataAdapter.receipt(order, result.get().getText());
                     }
                  }
